...
// elimina puntos y comas de la busqueda
$puntuacion = array(
    ',' => '', '.' => '', ';' => '', ':' => '', '"' => ''
);
...
for ($i = 0; $i < $numeroPaginas + 1; $i++) {
    ...
    // extrae los valores de las etiquetas y los asigna a una variable
    $cr->filter(".view-content .views-row")->each(function ($node) use (&$search, &$resultados) {
        ...
        //str_starts_with($titulotem,$search) valida si el primer parametro inicia con el segundo
        ...
        if (strcasecmp($search, $titulotem) == 0 ) {
